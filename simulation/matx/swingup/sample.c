/*
 *  Generated by matc (5.3.43); Tue Jul  4 18:47:01 2017
 */
...
Matrix z;
double r_max, theta_min;
double pre_r, pre_theta, pre_dt;
double l, M, m, a, f, g, c, J, n, k, E0;
#include <MaTXGVAL.h>
...
void var_init()
{
	...
	MatSetName(MatAssign(u, MatRowVec(1,
		0.0)), "u");
...
void on_task()
{
	...
	double Real_assign_tmp1;
	Matrix Matrix_assign_tmp1;
	Matrix sensor();
	void actuator();
	Matrix xh=MAT_DEF, xref=MAT_DEF;
	double r, theta, dr, dt, ddt;
	double sign_dtct, sat_ng;
	double E, v;
	double firstInputTime, secondInputTime, stopTime;

	MatSetName(MatAssign(y, sensor()), "y");

	(r = (double)(signed int)00);
	...
	if ( ((signed int)00 < MatGetVecValue(y, (signed int)2)) ) {
		while ( (PI < MatGetVecValue(y, (signed int)2)) ) {
			Real_assign_tmp1 = (MatGetVecValue(y, (signed int)2) - (2.0 * PI)),
			MatSetVecValue(y, (signed int)2, Real_assign_tmp1),
			Real_assign_tmp1;

			MatObjectTmpUndef();
		}
	} else {
		while ( (MatGetVecValue(y, (signed int)2) < (- PI)) ) {
			Real_assign_tmp1 = (MatGetVecValue(y, (signed int)2) + (2.0 * PI)),
			MatSetVecValue(y, (signed int)2, Real_assign_tmp1),
			Real_assign_tmp1;
	...
	(r = MatGetVecValue(y, (signed int)1));

	(theta = MatGetVecValue(y, (signed int)2));
	...
	if ( (((double)count * smtime) <= firstInputTime) ) {
		MatSetName(MatAssign(u, MatRowVec(1,
			15.0)), "u");

	} else if ( (((double)count * smtime) <= secondInputTime) ) {
		MatSetName(MatAssign(u, MatRowVec(1,
			(- 15.0))), "u");

	} else if ( (((double)count * smtime) <= stopTime) ) {
		MatSetName(MatAssign(u, MatRowVec(1,
			0.0)), "u");

	} else {
		if ( ((fabs(theta) <= ((theta_min / (double)(signed int)180) * PI)) && (fabs(r) < r_max)) ) {
			(isSwinging = (signed int)00);

		}
		if ( (isSwinging == (signed int)1) ) {
			(E = (((((J + ((m * l) * l)) * dt) * dt) / (double)(signed int)2) + (((m * g) * l) * (cos(theta) - (double)(signed int)1))));

			(sign_dtct = (double)(signed int)00);

			if ( ((dt * cos(theta)) < (signed int)00) ) {
				(sign_dtct = (double)(- (signed int)1));

			} else if ( ((signed int)00 < (dt * cos(theta))) ) {
				(sign_dtct = (double)(signed int)1);
			...
			(sat_ng = (((((- n) * g)) > (((((n * g)) < (((k * (E - E0)) * sign_dtct))) ? ((n * g)) : (((k * (E - E0)) * sign_dtct))))) ? (((- n) * g)) : (((((n * g)) < (((k * (E - E0)) * sign_dtct))) ? ((n * g)) : (((k * (E - E0)) * sign_dtct))))));

			(v = ((((- c) * dt) / ((m * l) * cos(theta))) + sat_ng));

			MatSetName(MatAssign(u, MatRowVec(1,
				(((((f * dr) - ((((m * l) * dt) * dt) * sin(theta))) + (((m * l) * ddt) * cos(theta))) + ((M + m) * v)) / a))), "u");

			if ( ((r_max < fabs(r)) && ((signed int)00 < (r * MatGetValue(u, (signed int)1, (signed int)1)))) ) {
				MatSetName(MatAssign(u, MatRowVec(1,
					(double)(signed int)00)), "u");
	...
	if ( (MatGetVecValue(u, (signed int)1) < (- 15.0)) ) {
		MatSetVecValue(u, (signed int)1, (- 15.0)),
		(- 15.0);

	} else if ( (20.0 < MatGetVecValue(u, (signed int)1)) ) {
		MatSetVecValue(u, (signed int)1, 15.0),
		15.0;
	...
	if ( ((cmd == (signed int)1) && (count < (signed int)5000)) ) {
		...
		Matrix_assign_tmp1 = MatRowVec(1,
			(double)isSwinging),
		MatSetSubMatrix2(data, (signed int)4, (signed int)4, int_sgn((signed int)4-(signed int)4), count, count, 1, Matrix_assign_tmp1),
		Matrix_assign_tmp1;
